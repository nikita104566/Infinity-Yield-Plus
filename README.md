...
One loadstring is enough. `lastcommand` is safe on a fresh install, and command history is saved in `IY_FE.iy` so ↑ / ↓ survive reload.

Older `hotfix7xx.luau` / `patches.luau` files are leftover and not required. If you still load them, they will not replace a newer version badge.
...
- Command history is saved across reload and rejoin in `IY_FE.iy` (`lastcommand`, ↑ / ↓)
- `;lastcommand` on a fresh install notifies instead of erroring
