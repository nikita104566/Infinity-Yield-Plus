...
One loadstring of `source` is enough for the core and for `showhistory` / `clearhistory`. Leftover `hotfix7xx.luau` / `patches.luau` files are optional extras and are not the product.
