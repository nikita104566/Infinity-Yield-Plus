...
Open the panel with your prefix (default `;`). One loadstring of `source` is enough.
...
- Automation Studio — run command workflows on spawn, chat, tools, prompts, and other events; click `$place` / `$job` / `$event` chips under the event title (they wrap onto a second row)
