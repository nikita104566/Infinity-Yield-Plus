...
Optional leftover `patches.luau` still exists for older hotfix commands that are not in `source` yet. Do not treat extra loadstrings as the product.
...
- `showhistory` / `hist` lists recent saved commands (in `source`)
