...
`openplugins` (`manageplugins`, `plugins`) opens the plugin list from the command panel after this one loadstring.

## Features

- Command panel with search, autocomplete, and a **Favorites** section at the top of the list
- `openplugins` / `manageplugins` / `plugins` opens the plugin list (Add / remove `.iy` files) without hunting through Settings
