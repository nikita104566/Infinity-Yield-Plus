...
One loadstring is enough. Older `hotfix7xx.luau` / `patches.luau` files are leftover compatibility loaders and are not required.

## Features

- Command panel with search, autocomplete, and a **Favorites** section at the top of the list
- Command names with aliases wrap to two lines so the primary name is not clipped
