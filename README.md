...
One loadstring is enough. Older `hotfix7xx.luau` / `patches.luau` files are optional leftovers and must not replace `source`.

## Features

- Command panel with search, autocomplete, and a **Favorites** section at the top of the list
- Favorited rows show a star; the list shows the primary name so long aliases do not clip the row
