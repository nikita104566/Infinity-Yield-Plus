...
### Added
- `listfavs` (`showfavs`, `favlist`) — notify with pinned favorite command names; empty list says nothing is pinned.

### Notes
- Lives in main `source`. One loadstring. EN/RU via T().
...
- `showhistory` (`hist`, `cmdhistory`) — shows the last few saved commands.
