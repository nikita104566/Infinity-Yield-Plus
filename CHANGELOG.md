...
### Improved
- `lastcommand` / `lastcmd` no longer errors on empty history — shows a short EN/RU notice.
- Command history is saved to and restored from `IY_FE.iy` with `source` alone.
- `showhistory` (`hist`, `cmdhistory`) lists recent saved commands; `clearhistory` (`clrhist`, `wipehistory`) wipes them.
- Meta-commands are not stored in ↑/↓ history.
- `hotfix762` no longer overwrites a newer in-source version badge.
