...
### Improved
- Command Palette (Ctrl+K) follows the panel language: title, search hint, section, empty state, and footer are EN+RU via `T()`.
- Palette search matches English and Russian descriptions; arguments after a description query stay on the selected command.
- Empty-result labels are cleared on each refresh so they do not stack.
