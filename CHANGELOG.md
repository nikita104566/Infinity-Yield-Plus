...
### Improved
- Command history is written into `IY_FE.iy` and restored after reload or rejoin (↑ / ↓ still work).
- `lastcommand` / `lastcmd` no longer errors when history is empty — it shows a short EN/RU notice instead.

### Notes
- One `source` loadstring is enough. Leftover `hotfix762` will not overwrite a 7.63+ badge.
