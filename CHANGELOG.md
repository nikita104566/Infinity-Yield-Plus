...
### Added
- `cmdpalette` (`palette`, `pal`) — opens the command palette from the panel or `;cmdpalette`, so you can search and run a command without scrolling the full list.

### Notes
- Lives in `source`. One loadstring is enough.
