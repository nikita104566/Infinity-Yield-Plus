...
### Improved
- Command list rows keep the primary name readable: only the first alias is shown, extra aliases collapse to +N.
- Typing a filter that matches nothing keeps the panel open and shows a localized empty line instead of a blank list.

### Notes
- One `source` loadstring is enough. Studio layout was not changed.
